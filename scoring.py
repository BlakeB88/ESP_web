...
def calculate_dual_meet_score(user_lineup_df, user_relay_df, opponent_lineup_df, opponent_relay_df):
    ...
    if not user_lineup_df.empty and not opponent_lineup_df.empty:
        ...
        if 'Event' not in user_lineup_df.columns:
            from utils import pivot_to_long_format
            user_lineup_df = pivot_to_long_format(user_lineup_df)
        if 'Event' not in opponent_lineup_df.columns:
            from utils import pivot_to_long_format
            opponent_lineup_df = pivot_to_long_format(opponent_lineup_df)
        
        # Get all events
        user_events = set(user_lineup_df['Event'].unique()) if 'Event' in user_lineup_df.columns else set()
        opp_events = set(opponent_lineup_df['Event'].unique()) if 'Event' in opponent_lineup_df.columns else set()
        all_events = user_events.union(opp_events)
        
        print("\nINDIVIDUAL EVENTS:")
        for event in sorted(all_events):
            user_event_data = user_lineup_df[user_lineup_df['Event'] == event] if 'Event' in user_lineup_df.columns else pd.DataFrame()
            opp_event_data = opponent_lineup_df[opponent_lineup_df['Event'] == event] if 'Event' in opponent_lineup_df.columns else pd.DataFrame()
            
            # Get times for this event
            user_times = list(user_event_data['Time']) if not user_event_data.empty and 'Time' in user_event_data.columns else []
            opp_times = list(opp_event_data['Time']) if not opp_event_data.empty and 'Time' in opp_event_data.columns else []
    ...
    return user_individual_pts + user_relay_pts, opponent_individual_pts + opponent_relay_pts